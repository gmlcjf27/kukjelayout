...
namespace _005_우체국_하나_CONVERT
{
    public class CONVERT
    {
        ...
        private static string strCardTypeName = "우체국_하나_컨버터";
        ...
        public static string ConvertRegister(string path, string xmlZipcodeAreaPath, string xmlZipcodePath)
        {
            ...
            try
            {
                ...
                while ((_strLine = _sr.ReadLine()) != null)
                {
                    ...
                    {
                        _strBranch = _encoding.GetString(_byteAry, 799, 2); //일반 : 25, 동의 : 50
                    ...
                    if (strDong_test == "I")
                    {
                        _sw = new StreamWriter(path + "_우체국_일반본인지정_1000", true, _encoding);
                    ...
                    else if (_strGong.ToUpper() == "D")
                    {
                        if (_strCodeType == "01")
                        {
                            _sw = new StreamWriter(path + "_우체국_공카드_3000", true, _encoding);
                            _sw.WriteLine(_strLine + "0054201");
                        }
                        else
                        {
                            _sw = new StreamWriter(path + "_우체국_공카드_2000", true, _encoding);
                    ...
                    //일반,동의서,지점 구분
                    //국제와 우체국 다른 코드
                    else if (_strBranch == "25")
                    {
                        _sw = new StreamWriter(path + "_우체국_일반_100", true, _encoding);
                        _sw.WriteLine(_strLine + "0051101");
                    }
                    //else if (_strBranch == "15")
                    //{
                    //    _sw = new StreamWriter(path + "_우체국_지점_1000", true, _encoding);
                    //    _sw.WriteLine(_strLine + "0054101");
                    //}
                    //else if (_strBranch == "16")
                    //{
                    //    _sw = new StreamWriter(path + "_우체국_지점_31000", true, _encoding);
                    ...
                    else
                    {
                        if (_strAgree_Type == "T")
                        {
                            if (_strCodeType == "01")
                            {
                                _sw = new StreamWriter(path + "_우체국_DONG_6500_T_CLUBSK", true, _encoding);
                                _sw.WriteLine(_strLine + "0052107");
                            }
                            else if (_strCodeType == "02")
                            {
                                _sw = new StreamWriter(path + "_우체국_DONG_7500_T_대한", true, _encoding);
                                _sw.WriteLine(_strLine + "0052108");
                            }
                            else if (_strCodeType == "03")
                            {
                                _sw = new StreamWriter(path + "_우체국_DONG_8500_T_아시아나", true, _encoding);
                                _sw.WriteLine(_strLine + "0052109");
                            }
                            else if (_strCodeType == "04")
                            {
                                _sw = new StreamWriter(path + "_우체국_DONG_9500_T_하이플러스", true, _encoding);
                                _sw.WriteLine(_strLine + "0052110");
                            }
                            else
                            {
                                _sw = new StreamWriter(path + "_우체국_DONG_5500_T_일반", true, _encoding);
                                _sw.WriteLine(_strLine + "0052106");
                            }
                        }
                        else if (_strAgree_Type == "B")
                        {
                            _sw = new StreamWriter(path + "_우체국_DONG_10500_법인_일반", true, _encoding);
                        ...
                        else
                        {
                            if (_strCodeType == "01")
                            {
                                _sw = new StreamWriter(path + "_우체국_DONG_1500_CLUB_SK", true, _encoding);
                                _sw.WriteLine(_strLine + "0052102");
                            }
                            else if (_strCodeType == "02")
                            {
                                _sw = new StreamWriter(path + "_우체국_DONG_2500_대한", true, _encoding);
                                _sw.WriteLine(_strLine + "0052103");
                            }
                            else if (_strCodeType == "03")
                            {
                                _sw = new StreamWriter(path + "_우체국_DONG_3500_아시아", true, _encoding);
                                _sw.WriteLine(_strLine + "0052104");
                            }
                            else if (_strCodeType == "04")
                            {
                                _sw = new StreamWriter(path + "_우체국_DONG_4500_하이플러스", true, _encoding);
                                _sw.WriteLine(_strLine + "0052105");
                            }
                            //2014.08.29 태희철 추가
                            else if (_strCodeType == "05")
                            {
                                _sw = new StreamWriter(path + "_우체국_DONG_11500_하이패스", true, _encoding);
                                _sw.WriteLine(_strLine + "0052112");
                            }
                            //2017.11.24 태희철 추가
                            else if (_strCodeType == "06")
                            {
                                _sw = new StreamWriter(path + "_우체국_DONG_14500_GS팝", true, _encoding);
                                _sw.WriteLine(_strLine + "0052115");
                            }
                            //2016.07.14 태희철 추가
                            else if (_strCodeType == "07")
                            {
                                _sw = new StreamWriter(path + "_우체국_DONG_12500_아모레퍼시픽", true, _encoding);
                                _sw.WriteLine(_strLine + "0052113");
                            }
                            //2017.04.20 태희철 추가
                            else if (_strCodeType == "08")
                            {
                                _sw = new StreamWriter(path + "_우체국_DONG_13500_더페이스샵", true, _encoding);
                                _sw.WriteLine(_strLine + "0052114");
                            }
                            //2018.05.02 태희철 추가
                            else if (_strCodeType == "09")
                            {
                                _sw = new StreamWriter(path + "_우체국_DONG_15500_청바G", true, _encoding);
                                _sw.WriteLine(_strLine + "0052116");
                            }
                            //2018.08.16 태희철 추가
                            else if (_strCodeType == "10")
                            {
                                _sw = new StreamWriter(path + "_우체국_DONG_16500_E1LPG", true, _encoding);
                                _sw.WriteLine(_strLine + "0052117");
                            }
                            //2018.11.13 태희철 추가
                            else if (_strCodeType == "11")
                            {
                                _sw = new StreamWriter(path + "_우체국_DONG_17500_등유나눔", true, _encoding);
                                _sw.WriteLine(_strLine + "0052118");
                            }
                            else
                            {
                                _sw = new StreamWriter(path + "_우체국_DONG_500_일반", true, _encoding);
                    ...
                    //{
                    //    _swTest = new StreamWriter(path + "_우체국오류_동의서_확인필요", true, _encoding);
