...
namespace bc_gift_inter
{
    public class CONVERT
    {
        //기본 인코딩 설정
        private static string strEncoding = "ks_c_5601-1987";
        private static string strCardTyepID = "043";
        private static string strCardTypeName = "BC기프트인터넷";
        ...
        {
            string _strReturn = "0011301";

            return _strReturn;
        }

        //등록 자료 생성
        //2013.06.27 태희철[S] 신주소 관련 수정
        ...
        public static string ConvertRegister(string path, string xmlZipcodePath, string xmlZipcodeAreaPath, string xmlZipcodePath_new, string xmlZipcodeAreaPath_new, string xmlPath)
        {
            ...
            string _strReturn = "";
            string _strLine = "";
            string _strZipcode = "", _strAreaType = "", _strAreaGroup = "", _strBranch = "", strNewAddress_Value = null;
            ...
            try
            {
                ...
                _dtable.Columns.Add("customer_office");                      // dr[15]
                _dtable.Columns.Add("customer_branch");
                _dtable.Columns.Add("card_product_code");                    // 발급구분코드
                //2013.06.27 태희철 추가 신주소관련[S]
                _dtable.Columns.Add("card_address_type1");                   // dr[18] 신주소구분자
                _dtable.Columns.Add("card_number");                          // dr[20]
                _dtable.Columns.Add("card_customer_regdate");                //카드등록일(실시간 사용)
                _dtable.Columns.Add("client_register_date");                 //dr[21] 카드생성일(실시간 사용)
                ...
                while ((_strLine = _sr.ReadLine()) != null)
                {
                    ...
                    _dr[7] = _encoding.GetString(_byteAry, 16, 6);
                    _dr[8] = _encoding.GetString(_byteAry, 22, 4);
                    _dr[9] = _encoding.GetString(_byteAry, 26, 20);
                    _dr[10] = _encoding.GetString(_byteAry, 66, 13);
                    _dr[11] = _encoding.GetString(_byteAry, 79, 12);
                    _dr[12] = _encoding.GetString(_byteAry, 91, 12);
                    _strZipcode = _encoding.GetString(_byteAry, 103, 6).Trim();
                    ...
                    _dr[13] = _strZipcode;
                    _dr[14] = _encoding.GetString(_byteAry, 109, 50);
                    _dr[15] = _encoding.GetString(_byteAry, 159, 40);
                    _dr[16] = _encoding.GetString(_byteAry, 199, 40);
                    _dr[17] = _encoding.GetString(_byteAry, 303, 1);

                    strNewAddress_Value = _encoding.GetString(_byteAry, 304, 1);

                    //카드번호
                    _dr[19] = _encoding.GetString(_byteAry, 0, 16);
                    _dr[20] = _encoding.GetString(_byteAry, 505, 8);  //카드등록일
                    _dr[21] = _encoding.GetString(_byteAry, 513, 8);  //카드발급일

                    //신주소
                    if (strNewAddress_Value != "1")
                    {
                        _dr[14] = _encoding.GetString(_byteAry, 305, 100).Trim() + _encoding.GetString(_byteAry, 405, 100).Trim();
                    }
        ...
        //기본 마감 자료
        public static string ConvertReceive(DataTable dtable, string fileName)
        {
            ...
            try
            {
                ...
                {
                    _strLine = GetStringAsLength(dtable.Rows[i]["client_send_date"].ToString(), 8);
                    _strLine += GetStringAsLength(dtable.Rows[i]["client_send_number"].ToString(), 8);
                    _strLine += GetStringAsLength(dtable.Rows[i]["card_denomination"].ToString(), 7);
                    _strLine += GetStringAsLength(dtable.Rows[i]["card_count"].ToString(), 6);
                    _strLine += GetStringAsLength(dtable.Rows[i]["card_bank_name"].ToString(), 21);
                    _strLine += GetStringAsLength(dtable.Rows[i]["card_bank_ID"].ToString(), 7);
                    _strLine += GetStringAsLength(dtable.Rows[i]["card_tel1"].ToString(), 13);
                    _strLine += GetStringAsLength(dtable.Rows[i]["card_zipcode"].ToString(), 7);
                    _strLine += GetStringAsLength(dtable.Rows[i]["card_address"].ToString(), 51);
