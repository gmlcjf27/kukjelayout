...
namespace _172GS
{
    public class CONVERT
    {
        ...
        private static string strCardTypeID = "138";
        private static string strCardTypeName = "GS칼텍스";
        //현 DLL의 카드 타입 코드 반환
        ...
        public static string ConvertRegister(string path, string xmlZipcodePath, string xmlZipcodeAreaPath, string xmlZipcodePath_new, string xmlZipcodeAreaPath_new, string xmlPath)
        {
            ...
            try
            {
                ...
                //세부 컬럼
                _dtable.Columns.Add("client_send_number");         // dr[5]
                ...
                _dtable.Columns.Add("card_tel1");                  // dr[10]
                ...
                while ((_strLine = _sr.ReadLine()) != null)
                {
                    ...
                    _dr[5] = _strAry[0];
                    _strZipcode = RemoveDash(_strAry[1]);
                    _dr[6] = _strZipcode;
                    _dr[7] = _strAry[2];
                    _dr[8] = _strAry[3];
                    _dr[9] = RemoveDash(_strAry[4]);
                    _dr[10] = RemoveDash(_strAry[5]);
                    _dr[11] = _strAry[6] + "^" + _strAry[7] + "^" + _strAry[8];
                    _dr[12] = _strAry[9];
                    _dr[13] = _strAry[0];   //일련번호와 동일

                    if (_strAry.LongLength != 10)
                    {
        ...
        public static string ConvertResult(System.Data.DataTable dtable, string fileName)
        {
            ...
            try
            {
                ...
                for (i = 0; i < dtable.Rows.Count; i++)
                {
                    ...
                    _strReceiver_CodeName = dtable.Rows[i]["receiver_code"].ToString();
                    _strReturnCode = dtable.Rows[i]["delivery_return_reason_last"].ToString();

                    _strLine = new StringBuilder(i + 1 + chCSV);
                    if (_strStatus == "1")
                    {   
                        _strLine.Append(GetStringAsLength(dtable.Rows[i]["card_in_date"].ToString(), 10) + chCSV);
                        _strLine.Append(GetStringAsLength(dtable.Rows[i]["client_send_number"].ToString(), 13) + chCSV);
                        _strLine.Append(GetStringAsLength(dtable.Rows[i]["customer_name"].ToString(), 20) + chCSV);
                        _strLine.Append(GetStringAsLength(dtable.Rows[i]["card_zipcode"].ToString(), 6) + chCSV);
                        _strLine.Append(GetStringAsLength(dtable.Rows[i]["receiver_name"].ToString(), 20) + chCSV);
                        _strLine.Append(GetStringAsLength(dtable.Rows[i]["receiver_SSN"].ToString(), 13) + chCSV);
                        _strLine.Append(GetStringAsLength(Receiver_Code_Name(_strReceiver_CodeName), 20) + chCSV);
                        _strLine.Append(GetStringAsLength("배송", 10) + chCSV);
                        _strLine.Append(GetStringAsLength(dtable.Rows[i]["card_delivery_date"].ToString(), 10) + chCSV);
                    }
                    else if (_strStatus == "2" || _strStatus == "3")
                    {
                        ...
                        _strLine.Append(GetStringAsLength(ReturnType(_strReturnCode), 20) + chCSV);
