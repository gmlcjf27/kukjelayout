...
namespace _004_삼성_일반_CONVERT
{
    public class CONVERT
    {
        ...
        private static string strCardTypeName = "삼성-일반-컨버터";
        ...
        public static string ConvertRegister(string path, string xmlZipcodeAreaPath, string xmlZipcodePath)
        {
            ...
            try
            {
                ...
                while ((_strLine = _sr.ReadLine()) != null)
                {
                    ...
                    _strCode = _encoding.GetString(_byteAry, 845, 2);

                    if (_strCode == "01")
                    {
                        _sw = new StreamWriter(path + ".1000_삼성일반_본인", true, _encoding);
                        _sw.WriteLine(_strLine + "0041102");
                    }
                    else
                    {
                        _sw = new StreamWriter(path + ".100_삼성일반", true, _encoding);
