...
namespace _002_국민_TEST
{
    public class CONVERT
    {
        ...
        private static string strCardTypeName = "국민카드_테스트";
            ...
            try
            {
                ...
                while ((_strLine = _sr.ReadLine()) != null)
                {
                    ...
                    if (_strSendNumber != _strSendNumberPrev)
                    {
                        ...
                        // 본인배송여부
                        if ((_strFamilyCode == "0" && _strOwner_one == "0") || (_strFamilyCode == "1" && _strFamily_one == "1"))
                        {
                            //동의서는 제외
                            ...
                            {
                                _dr[48] = _dr[48].ToString() + " / 본인지정배송(신분증확인필요)";
                            }
                            else
                            {
                                _dr[48] = "본인지정배송(신분증확인필요)";
                            }
                        }
        ...
        private static string ConvertReceiveType1(DataTable dtable, string fileName)
        {
            ...
            try
            {
                ...
                _sw11 = new StreamWriter(fileName + "국민KB증권_통합마감.txt", true, _encoding);
                _sw10 = new StreamWriter(fileName + "국민KB증권_통합마감_미배송_분실.txt", true, _encoding);
                _sw13 = new StreamWriter(fileName + "국민KB증권_통합마감_특송_일반_미처리.txt", true, _encoding);
                _sw15 = new StreamWriter(fileName + "국민KB증권_통합마감_특송_반송.txt", true, _encoding);
                _sw16 = new StreamWriter(fileName + "국민KB증권_통합마감_이미지파일.txt", true, _encoding);
                ...
                for (i = 0; i < dtable.Rows.Count; i++)
                {
                    ...
                    else
                    {
                        ...
                        DateTime dtDong_date = DateTime.Parse("2018-10-23");
                        ...
                        {
                            if (strCard_type_detail.Substring(0, 5) == "00226" && dtDong_date < CardInDate)
                            {
                            ...
                            if (_strStatus == "1" || _strStatus == "7")
                            {
                                ...
                                if (strCard_type_detail.Substring(0, 5) == "00226" && dtDong_date < CardInDate)
                                {
                                    ...
                                    if (_strStatus != "7")
                                    {
                                        ...
                                        //이미지파일명 저장
                                        if (strCard_type_detail.Substring(0, 5) == "00213")
                                        {
